...
{
    // Init window
    window.create(sf::VideoMode(window_width, window_height), "GameCorner");
    window.setVerticalSyncEnabled(true);
    window.setFramerateLimit(fps);

    sf::Image icon;

    if(!icon.loadFromFile("Textures/redToken.png"))
        std::cout << "Couldn't find image of icon" << std::endl;
    else
...
int Application::run()
{
    ...
    sf::Music theme;
    if (!theme.openFromFile("Audio/eeronmusat.wav"))
